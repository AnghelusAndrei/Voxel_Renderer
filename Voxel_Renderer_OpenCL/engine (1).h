...
SDL_Color FindPixel(float xa, float ya, int FOV, cl_int4 map[], int grid, cam Camera, int SCREEN_HEIGHT, int SCREEN_WIDTH, int res_x, int res_y, int density)
{
    ...
    return (SDL_Color){(Uint8)LimitColor(static_cast<int>(minVoxel.x-minDist*1-minShade)), (Uint8)LimitColor(static_cast<int>(minVoxel.y-minDist*1-minShade)), (Uint8)LimitColor(static_cast<int>(minVoxel.z-minDist*1-minShade)), 255};
}