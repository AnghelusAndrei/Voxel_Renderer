...
const uint32_t res_x = 400;
const uint32_t res_y = 400;
...
static int RenderEngine(void *data){
    ...
    err = clSetKernelArg(kernel, 6, sizeof(int), &FOV);
    ...
    cl_int4* houtput = new cl_int4[SCREEN_HEIGHT*SCREEN_WIDTH];
    ...
    while (SDL_AtomicGet(&running))
    {
        ...
        err = clSetKernelArg(kernel, 7, sizeof(cl_float3), &pos);
        err = clSetKernelArg(kernel, 8, sizeof(cl_float2), &ang);

        err = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_size, NULL, 0, NULL, NULL); 
        clFinish(queue);
        clEnqueueReadBuffer(queue, doutput, CL_TRUE, 0, SCREEN_HEIGHT*SCREEN_WIDTH*sizeof(cl_int4), houtput, 0, NULL, NULL);
