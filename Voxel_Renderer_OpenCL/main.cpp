...
static int RenderEngine(void *data){
    ...
    SDL_Color* map= new SDL_Color[grid*grid*grid];
    for(int z = 0; z < grid; z++){
        for(int y = 0; y < grid; y++){
            for(int x = 0; x < grid; x++){
                if(rand()%density==1){
                    map[z*grid*grid+y*grid+x] = (SDL_Color){(Uint8)(rand()%255), (Uint8)(rand()%255), (Uint8)(rand()%255)};
                }
            }   
        }
    }
    ...
    ddata = clCreateBuffer(context, CL_MEM_READ_ONLY, grid*grid*grid*sizeof(SDL_Color), NULL, NULL);
    doutput = clCreateBuffer(context, CL_MEM_WRITE_ONLY, SCREEN_HEIGHT*SCREEN_WIDTH*sizeof(SDL_Color), NULL, NULL);

    err = clEnqueueWriteBuffer(queue, ddata, CL_TRUE, 0, grid*grid*grid*sizeof(SDL_Color), map, 0, NULL, NULL);
    ...
    while (SDL_AtomicGet(&running))
    {
        ...
        for(float y = 0; y < SCREEN_HEIGHT; y+=SCREEN_HEIGHT/res_y){
            for(float x = 0; x < SCREEN_WIDTH; x+=SCREEN_WIDTH/res_x){
                SDL_Color pixel = FindPixel(x, y, FOV, map, grid, Camera, SCREEN_HEIGHT, SCREEN_WIDTH, res_x, res_y, density);
