...
static int RenderEngine(void *data){
    ...
    ddata = clCreateBuffer(context, CL_MEM_READ_ONLY, grid*grid*grid*sizeof(cl_int4), NULL, &err); CheckErr(err);
    doutput = clCreateBuffer(context, CL_MEM_WRITE_ONLY, res_y*res_x*sizeof(cl_int4), NULL, &err); CheckErr(err);

    err = clEnqueueWriteBuffer(queue, ddata, CL_TRUE, 0, grid*grid*grid*sizeof(cl_int4), map, 0, NULL, NULL); CheckErr(err);
    ...
    err = clSetKernelArg(kernel, 4, sizeof(cl_float), (void *)&rr); CheckErr(err);

    TTF_Init();
    TTF_Font * font = TTF_OpenFont("./utils/arial.ttf", 50);
    ...
    cl_int4* houtput = new cl_int4[res_x*res_y];
    ...
    while (SDL_AtomicGet(&running))
    {
        ...
        err = clSetKernelArg(kernel, 5, sizeof(cl_float3), (void *)&pos); CheckErr(err);
        err = clSetKernelArg(kernel, 6, sizeof(cl_float2), (void *)&ang); CheckErr(err);

        err = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_size, NULL, 0, NULL, NULL); CheckErr(err);
        err = clFinish(queue); CheckErr(err);
        err = clEnqueueReadBuffer(queue, doutput, CL_TRUE, 0, res_x*res_y*sizeof(cl_int4), houtput, 0, NULL, NULL); CheckErr(err);
        

        for(int y = 0; y < res_y; y++){
            for(int x = 0; x < res_x; x++){
                cl_int4 pixel = houtput[(int)(y*res_x+x)];
                ...
                SDL_Rect pixel_rect{x*SCREEN_WIDTH/res_x, y*SCREEN_HEIGHT/res_y, SCREEN_WIDTH/res_x, SCREEN_HEIGHT/res_y};
